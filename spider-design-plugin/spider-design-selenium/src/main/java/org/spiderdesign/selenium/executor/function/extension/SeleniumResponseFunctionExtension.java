package org.spiderdesign.selenium.executor.function.extension;
...
import org.spiderdesign.selenium.model.WebElements;
...
public class SeleniumResponseFunctionExtension implements FunctionExtension {
    ...
    @Comment("执行js")
    @Example("${resp.executeScript(\"document.write('hello spider-design !')\")}")
    public static Object executeScript(SeleniumResponse response, String script) {
        return executeScript(response, script, null);
    }

    @Comment("执行js")
    @Example("${resp.executeScript(\"document.write('hello '+arguments[0]+' !')\",\"spider-design\")}")
