...
public class UrlFunctionExecutor implements FunctionExecutor{
	...
	@Comment("获取url参数")
	@Example("${url.parameter('http://www.baidu.com/s?wd=spider-design','wd')}")
	public static String parameter(String url,String key){
		return parameterMap(url).get(key);
	}

	@Comment("获取url全部参数")
	@Example("${url.parameterMap('http://www.baidu.com/s?wd=spider-design&abbr=sf')}")
	...
	@Comment("url编码")
	@Example("${url.encode('http://www.baidu.com/s?wd=spider-design')}")
	public static String encode(String url){
		return encode(url,Charset.defaultCharset().name());
	}

	@Comment("url编码")
	@Example("${url.encode('http://www.baidu.com/s?wd=spider-design','UTF-8')}")
